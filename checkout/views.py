...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        if order_form.is_valid():
            ...
            # Associate the user profile if authenticated
            ...
            # Update order details based on delivery type
            if delivery_type == 'pickup':
                order.delivery_cost = 0.00 
                order.pick_up = True
                order.different_delivery_address = False
                print(f"Before saving order - Delivery Cost (Pickup): {order.delivery_cost}")
                ...
                print(f"Before saving order - Delivery Cost (different): {order.delivery_cost}")
                ...
                print(f"Before saving order - Delivery Cost (billing): {order.delivery_cost}")

            # Save bag and stripe details to the order
            pid = request.POST.get('client_secret').split('_secret')[0]
            order.stripe_pid = pid
            order.original_bag = json.dumps(bag)

            # Calculate grand total based on order total and delivery cost
            current_bag = bag_contents(request)
            order.order_total = Decimal(current_bag['total'])
            order.delivery_cost = Decimal(order.delivery_cost)
            order.grand_total = order.order_total + order.delivery_cost

            # Debugging prints to confirm final totals
    ...
    if request.method == 'POST':
        ...
        if order_form.is_valid():
            ...
            print(f"Before saving order - Delivery Cost (grand): {order.delivery_cost}")
            order.save()
...
def checkout_success(request, order_number):
    ...
    print(f"Checkout Success - Delivery Cost for Order {order.order_number}: {order.delivery_cost}")
